...
export function AppShell() {
    ...
    useEffect(() => {
        if (isDm || channelId || !server)
            return;
        const target = firstChannelId(server);
        if (target)
            navigate(`/channels/${server.id}/${target}`, { replace: true });
    }, [channelId, isDm, navigate, server, serverId]);
    return (_jsxs(_Fragment, { children: [_jsx(ConnectionBanner, { state: connection }), layout === 'mobile' ? _jsx(MobileLayout, {}) : null, layout === 'tablet' ? _jsx(TabletLayout, {}) : null, layout === 'desktop' ? _jsx(DesktopLayout, {}) : null] }));
}
