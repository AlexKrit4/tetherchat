...
export function useRealtime() {
    ...
    useEffect(() => {
        ...
        const onMessageDeleted = ({ messageId, channelId }) => {
            messageCache.removeMessage(client, channelId, messageId);
        };
        const onReactionUpdated = ({ messageId, channelId, reactions, }) => {
            const existing = client
                .getQueryData(queryKeys.messages(channelId))
        ...
        const onTypingUpdate = ({ channelId, users, }) => {
            setTyping(channelId, users.filter((user) => user.id !== currentUserId));
        };
        const onPresence = ({ userId, status: next }) => {
            setPresence(userId, next);
        };
        const onMemberJoin = ({ serverId, member }) => {
            client.setQueryData(queryKeys.members(serverId), (current) => current ? [...current.filter((row) => row.userId !== member.userId), member] : current);
            void client.invalidateQueries({ queryKey: queryKeys.servers });
        };
        const onMemberLeave = ({ serverId, userId }) => {
            client.setQueryData(queryKeys.members(serverId), (current) => current?.filter((row) => row.userId !== userId));
            if (userId === currentUserId)
                void client.invalidateQueries({ queryKey: queryKeys.servers });
        };
        const onMemberUpdate = ({ serverId, member }) => {
            client.setQueryData(queryKeys.members(serverId), (current) => current?.map((row) => (row.userId === member.userId ? member : row)));
        };
        const invalidateServer = (serverId) => {
            void client.invalidateQueries({ queryKey: queryKeys.server(serverId) });
        };
        const onDmCreate = (conversation) => {
            client.setQueryData(queryKeys.dms, (current) => current ? [conversation, ...current.filter((row) => row.id !== conversation.id)] : current);
        };
...
function markChannelUnread(client, channelId, mentioned) {
    client.setQueryData(queryKeys.readStates, (current) => {
        ...
        return current.map((state) => state.channelId === channelId
            ? {
                ...state,
                unread: true,
