...
class AppViewModel(application: Application) : AndroidViewModel(application), DefaultLifecycleObserver {
  ...
  var sessions by mutableStateOf<List<DeviceSession>>(emptyList())
    private set
  var pendingQrTicket by mutableStateOf<String?>(null)
    private set
  ...
  fun promptQrLogin(ticket: String) {
    pendingQrTicket = ticket.trim()
  }

  fun dismissQrLoginPrompt() {
    pendingQrTicket = null
  }

  fun approveQrLogin(ticket: String, onSuccess: () -> Unit = {}) {
    viewModelScope.launch {
      busy = true
      runCatching { withContext(Dispatchers.IO) { api.approveQrLogin(ticket) } }
        .onSuccess {
          pendingQrTicket = null
          error = "Вход на компьютере подтверждён"
          onSuccess()
        }
        .onFailure { error = it.userMessage() }
      busy = false
  ...
  fun onQrCodeScanned(raw: String) {
    val ticket = parseQrLoginTicket(raw)
    if (ticket == null) {
      error = "Не удалось распознать QR-код входа"
      return
    }
    pendingQrTicket = ticket
  }
...
internal fun parseQrLoginTicket(raw: String): String? {
  val trimmed = raw.trim()
  if (trimmed.length in 20..128 && !trimmed.contains("://") && !trimmed.contains(' ')) return trimmed
  ...
  return ticket.takeIf { it.length >= 10 }
}
