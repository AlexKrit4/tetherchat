...
class RealtimeClient(
  ...
  fun connect(handlers: RealtimeHandlers) {
    ...
    next.on("call:ring") { args -> decode<CallRingPayload>(args)?.let(handlers.onCallRing) }
    next.on("call:accepted") { args -> decode<CallSignalPayload>(args)?.let(handlers.onCallAccepted) }
    next.on("call:declined") { handlers.onCallEnded() }
    next.on("call:ended") { handlers.onCallEnded() }
    next.on("call:busy") { handlers.onCallEnded() }
    next.on("call:missed") { handlers.onCallEnded() }
...
class RealtimeHandlers(
  ...
  val onCallRing: (CallRingPayload) -> Unit,
  val onCallAccepted: (CallSignalPayload) -> Unit,
