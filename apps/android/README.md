...
Release builds are signed with `secrets/tetherchat-release.jks` (see `keystore.properties.example`).
Keep that keystore backed up on the server — without it, users cannot update in place.
...
Closed-app delivery uses `app/google-services.json` plus `FCM_SERVICE_ACCOUNT_JSON`
on the API (the private key that *sends* FCM). Public client ids are also exposed
at `GET /api/push/android-config` as a fallback. Add the debug SHA-1 below to the
